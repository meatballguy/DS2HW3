    ...
    for (string fileNum : {"301", "302", "303", "304", "305"}) {
        ...
        for(auto& st : data) {
            ...
            while(count < tableSize) {
                ...
                pos = (pos + factor * factor) % tableSize; // WRONG FORMULA
                factor++; count++;
            }
        }
        ...
        for (int i=0; i<tableSize; i++) {
            int pos = i; int factor = 1; int count = 1;
            while(count <= tableSize) {
                if(table[pos].state == EMPTY) { sum1 += (count-1); break; }
                pos = (pos + factor * factor) % tableSize;
                factor++; count++;
            }
        }
        cout << fileNum << ": sum(count)=" << sum << " sum(count-1)=" << sum1 << endl;
    }
}
